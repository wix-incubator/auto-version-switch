  ...
  try {
    if (cluster.isMaster) {
      ...
      fetchExpectedVersion(function (err, version) {
        if (err) {
          return process.send({code: 'runner-failed-fetch-expected-version', err: err, errstack: err.stack});
        }
        ...
        try {
          ...
          process.send({code: 'runner-failed-switch-version', err: err, errstack: err.stack});
        }
      });
    }
  } catch (er) {
  ...
  fetchExpectedVersion(function (err, version) {
    ...
    worker.on('message', function (message) {
      workerDebug(worker, 'received cluster message: [' + message.code + ']' +
        (message.err ? ', err:' + message.err + ', stack: ' + message.errstack : ''));
      ...
      else if (message.code.indexOf('runner-failed') === 0) {
        error(message.err, message.errstack);
        worker.kill();
        process.exit(1);
      }
    });
...
function error(err, errstack) {
  if (err) {
    var stack = errstack ? errstack : (err && err.stack ? err.stack : new Error('[synthetic stack]').stack);
